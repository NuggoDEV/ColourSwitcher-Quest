...
foreach ($fileName in $modFiles) {
    & adb shell mkdir /sdcard/TempMods
    if ($useDebug -eq $true) {
        & adb push build/debug/$fileName /sdcard/TempMods/$fileName
    } else {
        & adb push build/$fileName /sdcard/TempMods/$fileName
    }
    & adb shell cp /sdcard/TempMods/$fileName /sdcard/Android/data/com.beatgames.beatsaber/files/mods
    & adb shell rm -rf /sdcard/TempMods/*
    & adb shell chmod 777 /sdcard/Android/data/com.beatgames.beatsaber/files/mods/*
}
